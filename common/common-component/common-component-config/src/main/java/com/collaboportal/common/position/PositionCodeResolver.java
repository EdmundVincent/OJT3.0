...
public final class PositionCodeResolver {
    ...
    static {
        PositionCodeConfig cfg = CommonConfigFactory.createConfig(PositionCodeConfig.class);
        Map<String, String> m = cfg.getRules();
        if (m == null || m.isEmpty()) {
            throw new IllegalStateException("役職ルールが見つかりません");
        }
        RAW_RULES = Collections.unmodifiableMap(new HashMap<>(m));
    }
    public static PositionRule get(String jobCode) { return get(jobCode, null); }
    public static PositionRule get(String jobCode, String context) {
        ...
        if (rawCandidate == null) rawCandidate = RAW_RULES.get(jobCode);
        if (rawCandidate == null) rawCandidate = RAW_RULES.get("other");
        if (rawCandidate == null) throw new IllegalStateException(jobCode + "のルール存在しません。");
    ...
    private static PositionRule parse(String raw) {
        String[] lr = raw.split("\\|");
        if (lr.length != 2) throw new IllegalArgumentException("Bad rule: " + raw);
    ...
    private static int bitsToMask(String csv) {
        ...
        if (arr.length != 5) throw new IllegalArgumentException("ルール不具合" + csv);
        int mask = 0;
        for (int i = 0; i < 5; i++) {
            if ("1".equals(arr[i].trim())) mask |= (1 << i);
        }
        return mask;
    }
    public static Map<String, String> snapshot() { return RAW_RULES; }
}
